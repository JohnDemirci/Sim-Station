...
struct CreateSimulatorDeviceTypesView: View {
	...
	private let deviceTypes: [DeviceType]

    init(createSimulatorFeature: CreateSimulatorFeature, deviceTypes: [DeviceType]) {
	...
	var body: some View {
		VStack(spacing: 24) {
			HeaderView()
			DeviceModelSelectionContentView(deviceTypes: deviceTypes)
		}
...
private struct HeaderViewIconView: View {
	...
	var body: some View {
		Image(systemName: createSimulatorFeature.iconSystemImage)
			...
			.animation(.spring, value: createSimulatorFeature.iconSystemImage)
			.transition(.scale)
	}
}
...
private struct DeviceModelSelectionContentView: View {
	...
	var body: some View {
		VStack(spacing: 16) {
			...
			VStack(spacing: 8) {
				ContentPickerView(deviceTypes: deviceTypes)
				SelectedDeviceTypeView()
			}
			.padding(.bottom, 16)
		}
...
private struct ContentPickerView: View {
	...
	var body: some View {
        Picker("Device Model", selection: createSimulatorFeature.binding(\.selectedDeviceType) { sdt in
            .selectDeviceType(sdt)
        }) {
...
private struct SelectedDeviceTypeView: View {
	...
	var body: some View {
		HStack {
			...
			Text("Selected: \(createSimulatorFeature.selectedDeviceType?.name ?? "")")
				.font(.caption)
				.foregroundColor(.secondary)
		...
		.padding(.horizontal, 16)
		.padding(.bottom, 8)
		.inCase(createSimulatorFeature.selectedDeviceType != nil) {
			EmptyView()
		}
	}
}
...
private struct PreviousButtonToolbarView: ToolbarContent {
	...
	var body: some ToolbarContent {
		ToolbarItem(placement: .cancellationAction) {
			Button("Previous") {
                createSimulatorFeature.send(.navigate(.runtimes))
			}
...
private struct NextButtonToolbarView: ToolbarContent {
	...
	var body: some ToolbarContent {
		ToolbarItem(placement: .confirmationAction) {
			...
			.buttonStyle(.borderedProminent)
			.disabled(createSimulatorFeature.selectedDeviceType == nil)
