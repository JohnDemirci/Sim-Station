...
struct CreateSimulatorSuccessView: View {
	...
	var body: some View {
		VStack(spacing: 32) {
			...
			PostCreationSimulatorInfoView(successAnimation: successAnimation)

			Spacer()

			PostCreationActionView(successAnimation: successAnimation)
		}
	...
	var body: some View {
		VStack(spacing: 8) {
			SimulatorDetailsRowView(
				icon: "iphone",
				title: "Device",
				value: createSimulatorFeature.selectedDeviceType?.name ?? ""
			)

			SimulatorDetailsRowView(
				icon: "gear",
				title: "Runtime",
				value: createSimulatorFeature.selectedRuntime?.name ?? ""
			)

			SimulatorDetailsRowView(
				icon: "tag",
				title: "Name",
				value: createSimulatorFeature.selectedName
			)
		}
		.padding(.horizontal, 16)
		.padding(.bottom, 16)
	...
	var body: some View {
		VStack(spacing: 12) {
			Button("Create Another Simulator") {
				createSimulatorFeature.send(.reset)
			}
