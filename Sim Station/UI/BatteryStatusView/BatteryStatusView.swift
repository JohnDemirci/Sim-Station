...
struct BatteryStatusView: View {
    ...
    @State private var batteryStatusFeatureState: BatteryStatusFeatureState = .idle
    ...
    var body: some View {
        FeatureStateView(state: $batteryStatusFeatureState) { feature in
            BatteryStatusLoadableView()
                .environment(feature)
        }
        .instantiate(with: container.batteryStatusFeature(simulatorID))
    }
}

private struct BatteryStatusLoadableView: View {
	@Environment(WindowSceneFeature.self) private var windowSceneFeature
    @Environment(BatteryStatusFeature.self) private var batteryStatusFeature

	var body: some View {
        LoadableValueView(
            batteryStatusFeature.savedState,
            loaded: { _ in BatteryStatusMainView() },
            failed: { error in Text(error.localizedDescription) }
        )
		.task {
			batteryStatusFeature.send(.retrieveCurrentState)
		}
		.onDisappear {
			windowSceneFeature.send(.openBatteryStatus(nil))
		}
	}
}
			...
			ZStack(alignment: .leading) {
				...
				RoundedRectangle(cornerRadius: 4)
					.fill(
						LinearGradient(
							colors: [
                                batteryStatusFeature.batteryColor,
                                batteryStatusFeature.batteryColor.opacity(0.7)
							],
							startPoint: .leading,
							endPoint: .trailing
						)
					...
					.frame(
						width: max(4, CGFloat(batteryStatusFeature.level) * 0.92),
						height: 42
					)
					.offset(x: 4, y: 0)
					.animation(.easeInOut(duration: 0.8), value: batteryStatusFeature.level)
			}
...
private struct BatteryControlView: View {
	...
	var body: some View {
		VStack(spacing: 20) {
			BatteryLevelControlView()
            BatteryChargeStateView(currentCharge: batteryStatusFeature.binding(\.chargeState) { value in
                .updateBatteryChargeState(value)
            })
		}
		.padding()
	}
		...
		VStack(spacing: 16) {
			HStack {
				Text("Battery Level")
					...
					.foregroundColor(.primary)
				Spacer()
				Text("\(batteryStatusFeature.level)%")
					.font(.headline)
					.foregroundColor(.secondary)
					.contentTransition(.numericText())
					.animation(.easeInOut(duration: 0.3), value: batteryStatusFeature.level)
			}

			ModernSlider(
                value: batteryStatusFeature.binding(\.level) { level in
                    .updateLevel(level)
                },
				initialValue: Double(batteryStatusFeature.level)
			)
		}
		.padding(20)
		.background(
...
private struct UpdateButtonView: View {
	...
	var body: some View {
		Button(action: {
			batteryStatusFeature.send(.setNewBatteryState)
		}) {
			HStack {
